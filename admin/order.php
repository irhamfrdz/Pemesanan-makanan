...
if (isset($_POST['kirim'])) {
	$updatestatus = mysqli_query($conn, "update cart set status='Serah Terima' where orderid='$orderids'");
...
if (isset($_POST['verifikasi'])) {
	$updatestatus = mysqli_query($conn, "update cart set status='Diverifikasi' where orderid='$orderids'");

	if ($updatestatus) {
		echo " <div class='alert alert-success'>
			<center>Pembayaran Diverifikasi.</center>
		  </div>
		<meta http-equiv='refresh' content='1; url= manageorder.php'/>  ";
	} else {
		echo "<div class='alert alert-warning'>
...
<head>
	...
	<title>Dhan MOTOR - Pesanan <?php echo $checkdb['namalengkap']; ?></title>
					...
					<div class="col-12">
						<div class="card">
							<div class="card-body">
								...
								<br>
								<?php
								if ($checkdb['status'] == 'Menunggu Verifikasi Pembayaran') {
								...
								} elseif ($checkdb['status'] == 'Diverifikasi') {
									echo '
											<form method="post">
												<input type="submit" name="kirim" class="form-control btn btn-success" value="Proses Serah Terima" />
											</form>
										';
								} else {
									echo '
