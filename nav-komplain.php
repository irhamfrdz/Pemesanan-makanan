...
if(!isset($_SESSION['log'])){
	...
	$uid = $_SESSION['id'];
	$caricart = mysqli_query($conn,"select * from cart where userid='$uid' and status='Complain'");
	...
	$itungtrans = mysqli_query($conn,"select count(orderid) as jumlahtrans from cart where userid='$uid' and status='Complain'");
...
<html>
<head>
<title>Fikry Catering - Daftar Belanja</title>
<!-- for-mobile-apps -->
			...
			<div class="checkout-right">
				<table class="timetable_sub">
					...
					<?php 
						...
						$brg=mysqli_query($conn,"SELECT DISTINCT(idcart), c.orderid, tglorder, status from cart c, detailorder d where c.userid='$uid' and d.orderid=c.orderid and status='Complain' order by tglorder DESC");
						$no=1;
					...
					<tr class="rem1"><form method="post">
						...
						<td class="invert">
							<div class="rem">
								<?php
								if($b['status']=='Complain'){
								echo '
								<a href="lihat-detail-komplain.php?id='.$b['orderid'].'" class="form-control btn-primary">
								Lihat Detai Complain
								</a>
								';} else {
									echo '-';
								}
